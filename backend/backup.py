...
def save_address_book(data, host_name, ca_bundle=None):
    for i in data:
        ...
        response = requests.put(url, data=json.dumps(body).encode('utf-8'), verify=ca_bundle)
        response.raise_for_status()


def save_reminders(data, host_name, ca_bundle):
    for i in data:
...
def do_restore(in_file, host_name, ca_bundle=None):
    with open(in_file, "rb") as f:
        data = json.load(f)
    
    save_address_book(data["address_book"], host_name, ca_bundle)
    save_reminders(data["reminders"], host_name, ca_bundle)
...
def main():
    ...
    try:
        if args.command == "backup":
            ...
            do_backup(args.host_name, args.ca_bundle, args.output_file)
        ...
        if args.command == "restore":
            ...
            do_restore(args.input_file, args.host_name, args.ca_bundle)
