...
func anniversaryRefTimeGen(r *repo.Reminder, now time.Time) time.Time {
	...
	h := r.Spec
	refThisYear := time.Date(now.Year(), h.Month(), h.Day(), 0, 0, 0, 0, time.UTC)
	var offset int
	...
	refThisYear = time.Date(refThisYear.Year(), refThisYear.Month(), refThisYear.Day(), h.Hour(), h.Minute(), 0, 0, time.UTC)
...
func weeklyRefTimeGen(r *repo.Reminder, now time.Time) time.Time {
	...
	now = time.Date(now.Year(), now.Month(), now.Day(), h.Hour(), h.Minute(), 0, 0, time.UTC)
